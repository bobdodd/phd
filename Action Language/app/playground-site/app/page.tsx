  ...
  const detectIssues = (nodes: any[], currentFiles: ExampleFiles) => {
    ...
    if (allHtml.trim().length > 0) {
      try {
        ...
        // Run ColorContrastAnalyzer (needs DocumentModel)
        const colorContrastAnalyzer = new ColorContrastAnalyzer();
        const colorContrastIssues = colorContrastAnalyzer.analyze({
          documentModel,
          actionLanguageModel,
          scope: 'file'
        });
        ...
        // Run LiveRegionAnalyzer (needs DocumentModel)
        const liveRegionAnalyzer = new LiveRegionAnalyzer();
        const liveRegionIssues = liveRegionAnalyzer.analyze({
          documentModel,
          actionLanguageModel,
          scope: 'file'
        });
        ...
        // TimeoutAnalyzer - WCAG 2.2.1 (Timing Adjustable)
        const timeoutAnalyzer = new TimeoutAnalyzer();
        const timeoutIssues = timeoutAnalyzer.analyze({
          documentModel,
          actionLanguageModel,
          scope: 'file'
        });

        for (const issue of timeoutIssues) {
          detected.push({
            type: issue.type,
            severity: issue.severity,
            wcag: issue.wcagCriteria || [],
        ...
        // PointerTargetAnalyzer - WCAG 2.5.5 (Target Size Level AAA), 2.5.8 (Level AA)
        const pointerTargetAnalyzer = new PointerTargetAnalyzer();
        const pointerTargetIssues = pointerTargetAnalyzer.analyze({
          documentModel,
          actionLanguageModel,
          scope: 'file'
        });

        for (const issue of pointerTargetIssues) {
          detected.push({
            type: issue.type,
            severity: issue.severity,
            wcag: issue.wcagCriteria || [],
