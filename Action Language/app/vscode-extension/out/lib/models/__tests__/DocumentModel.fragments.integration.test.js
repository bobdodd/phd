...
const ActionLanguageModel_1 = require("../ActionLanguageModel");
/**
 * Helper to create a test DOM element
 */
function createElement(id, tagName = 'div', attributes = {}) {
    const location = {
        file: 'test.html',
        line: 1,
        column: 0
 ...
 */
function createFragment(rootId, children) {
    const root = createElement(rootId, 'div');
    root.children = children;
    for (const child of children) {
        child.parent = root;
    }
    return new DOMModel_1.DOMModelImpl(root, `${rootId}.html`);
}
/**
 * Helper to create an ActionLanguage event handler node
 */
function createEventHandler(elementId, event, sourceFile = 'test.js') {
    const location = {
        file: sourceFile,
        line: 10,
        column: 0
...
describe('DocumentModel Multiple Fragments Integration', () => {
    ...
    describe('Event Handler Merging Across Fragments', () => {
        it('should attach handlers to elements across fragments', () => {
            ...
            const jsModel = new ActionLanguageModel_1.ActionLanguageModelImpl([handler], 'handlers.js');
            const model = new DocumentModel_1.DocumentModel({
                scope: 'page',
                dom: [fragment],
                javascript: [jsModel],
        ...
        it('should attach handlers from multiple JS files to same element', () => {
            ...
            const jsModel1 = new ActionLanguageModel_1.ActionLanguageModelImpl([clickHandler], 'click.js');
            // JavaScript 2: Keyboard handler
            const keyHandler = createEventHandler('submit-btn', 'keydown', 'keyboard.js');
            const jsModel2 = new ActionLanguageModel_1.ActionLanguageModelImpl([keyHandler], 'keyboard.js');
            const model = new DocumentModel_1.DocumentModel({
                scope: 'page',
                dom: [fragment],
    ...
    describe('Interactive Elements Across Fragments', () => {
        it('should find interactive elements from all fragments', () => {
            ...
            const jsModel = new ActionLanguageModel_1.ActionLanguageModelImpl([handler1, handler2], 'handlers.js');
            const model = new DocumentModel_1.DocumentModel({
                scope: 'page',
                dom: [fragment1, fragment2],
                javascript: [jsModel],
    ...
    describe('Real-World Component Fragment Scenarios', () => {
        it('should handle typical React component development scenario', () => {
            ...
            const jsModel = new ActionLanguageModel_1.ActionLanguageModelImpl([openHandler, closeHandler, escapeHandler], 'modal.js');
            const model = new DocumentModel_1.DocumentModel({
                scope: 'page',
        ...
        it('should show improving confidence as fragments merge', () => {
            ...
            const fragments1 = Array.from({ length: 9 }, (_, i) => createFragment(`comp${i}`, [createElement(`elem${i}`, 'div')]));
            const model1 = new DocumentModel_1.DocumentModel({
                scope: 'page',
                dom: fragments1,
                javascript: [],
                css: []
            ...
            const fragments2 = Array.from({ length: 3 }, (_, i) => createFragment(`comp${i}`, [createElement(`elem${i}`, 'div')]));
            const model2 = new DocumentModel_1.DocumentModel({
                scope: 'page',
                dom: fragments2,
                javascript: [],
                css: []
            ...
            const model3 = new DocumentModel_1.DocumentModel({
                scope: 'page',
                dom: [fragment3],
                javascript: [],
                css: []
...
//# sourceMappingURL=DocumentModel.fragments.integration.test.js.map