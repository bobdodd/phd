...
export class FocusManagementAnalyzer extends BaseAnalyzer {
  ...
  readonly description =
    'Detects focus management issues that can strand keyboard users';
  ...
  private detectRemovalWithoutFocusCheck(
    model: ActionLanguageNode[] | { nodes: ActionLanguageNode[] },
        ...
        if (!hasFocusCheck) {
          ...
          issues.push(
            this.createIssue(
              ...
              message,
              node.location,
              ['2.4.3', '2.4.7'], // WCAG 2.4.3: Focus Order, 2.4.7: Focus Visible
              context,
              { fix }
            )
          );
    ...
    for (const node of nodes) {
      ...
      if (node.actionType === 'focusChange' && node.timing === 'immediate') {
        ...
        if (element) {
          ...
          const looksNaturallyFocusable = naturallyFocusableTags.some(tag =>
  ...
  private detectMissingFocusRestoration(
    ...
    for (const node of nodes) {
      if (
        ...
        const looksLikeDialogClose =
          node.metadata?.role === 'dialog' || // Explicit dialog role
  ...
  private hasFocusCheckNearby(
    ...
    for (let i = rangeStart; i < rangeEnd; i++) {
      const node = nodes[i];
      if (
        node.metadata?.object === 'document.activeElement' ||
  ...
  private hasFocusRestorationNearby(
    ...
    for (let i = rangeStart; i < rangeEnd; i++) {
      ...
      if (
        binding.includes('previousfocus') ||
   ...
   */
  private isElementFocusable(element: DOMElement): boolean {
   ...
   */
  private generateRemovalFix(node: ActionLanguageNode): IssueFix {
...
element.remove();`,
   ...
   */
  private generateHidingFix(node: ActionLanguageNode): IssueFix {
    const selector = node.element.selector;
