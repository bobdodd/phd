...
export class CSSParser {
  ...
  parse(source: string, sourceFile: string): CSSModelImpl {
    ...
    return new CSSModelImpl(rules, sourceFile);
  }

  /**
   * Extract a CSS rule from AST node.
   */
  private extractRule(node: any, sourceFile: string): CSSRule | null {
   ...
   */
  private extractProperties(block: any): CSSProperties {
    const properties: CSSProperties = {};

    if (!block.children) return properties;
  ...
  private detectPseudoClass(
    selector: string
  ): 'hover' | 'focus' | 'active' | 'focus-visible' | 'focus-within' | 'disabled' | 'checked' | undefined {
    if (selector.includes(':focus-visible')) return 'focus-visible';
    if (selector.includes(':focus-within')) return 'focus-within';
  ...
  private analyzeAccessibilityImpact(
    ...
    return {
      ...
      affectsVisibility: propertyKeys.some((key) =>
        visibilityProperties.includes(key)
      ),
      affectsContrast: propertyKeys.some((key) => contrastProperties.includes(key)),
      affectsInteraction: propertyKeys.some((key) =>
        interactionProperties.includes(key)
      ),
   ...
   */
  private extractLocation(node: any, sourceFile: string): SourceLocation {
    if (node.loc) {
      return {
        file: sourceFile,
