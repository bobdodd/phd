...
export class WidgetPatternAnalyzer extends BaseAnalyzer {
  readonly name = 'WidgetPatternAnalyzer';
  readonly description =
    'Validates complete implementation of 21 WAI-ARIA widget patterns (tabs, dialogs, menus, etc.)';
  ...
  analyze(context: AnalyzerContext): Issue[] {
    ...
    if (context.documentModel && context.documentModel.javascript.length > 0) {
      ...
      // File-scope fallback
      issues.push(
        ...this.analyzeWidgetPatterns(context.actionLanguageModel.nodes, context)
      );
    }
  ...
  private analyzeWidgetPatterns(
    ...
    const roleElements = new Map<string, ActionLanguageNode[]>();

    // Collect all role assignments
    for (const node of nodes) {
      if (
        node.actionType === 'ariaStateChange' &&
  ...
  <button role="tab" aria-selected="false" aria-controls="panel2">Tab 2</button>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Tablist at ${elementSelector} missing child tabs with role="tab"`,
            tablist.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
    ...
    if (tabs[nextIndex]) tabs[nextIndex].focus();
        ...
        issues.push(
          this.createIssue(
            'incomplete-tabs-pattern',
            'warning',
            `Tablist at ${elementSelector} missing arrow key navigation`,
            tablist.location,
            ['2.1.1', '4.1.2'],
            context,
            { fix }
  ...
  private validateDialogPattern(
    ...
    for (const dialog of dialogs) {
      ...
      if (!hasAriaModal) {
        ...
        issues.push(
          this.createIssue(
            ...
            'error',
            `Dialog at ${elementSelector} missing aria-modal="true"`,
            dialog.location,
            ['4.1.2'],
            context,
            { fix }
      ...
      const hasEscapeHandler = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
          n.event === 'keydown' &&
    ...
    closeDialog();
        ...
        issues.push(
          this.createIssue(
            'incomplete-dialog-pattern',
            'warning',
            `Dialog at ${elementSelector} missing Escape key handler`,
            dialog.location,
            ['2.1.2'],
            context,
            { fix }
      ...
      e.preventDefault();
        ...
        issues.push(
          this.createIssue(
            'incomplete-dialog-pattern',
            'error',
            `Dialog at ${elementSelector} missing focus trap implementation`,
            dialog.location,
            ['2.1.2'],
            context,
            { fix }
  ...
  private validateAccordionPattern(
    ...
    const expandedElements = nodes.filter(
      (n) =>
        n.actionType === 'ariaStateChange' &&
    ...
    for (const element of expandedElements) {
      ...
      const hasAriaControls = nodes.some(
        (n) =>
          n.actionType === 'ariaStateChange' &&
...
<div id="panel1" hidden>
  Panel content
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Accordion button at ${elementSelector} missing aria-controls attribute`,
            element.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
      ...
      const hasToggleHandler = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
  ...
  panel.hidden = isExpanded;
        ...
        issues.push(
          this.createIssue(
            'incomplete-accordion-pattern',
            'warning',
            `Accordion button at ${elementSelector} missing click handler`,
            element.location,
            ['2.1.1'],
            context,
            { fix }
        ...
        if (!hasAttr) {
          ...
          issues.push(
            this.createIssue(
              ...
              'error',
              `Combobox at ${elementSelector} missing required ${attr}`,
              combobox.location,
              ['4.1.2'],
              context,
              { fix }
    ...
    // Update aria-activedescendant
        ...
        issues.push(
          this.createIssue(
            'incomplete-combobox-pattern',
            'warning',
            `Combobox at ${elementSelector} missing arrow key navigation`,
            combobox.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  <div role="menuitem">Menu Item 3</div>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Menu at ${elementSelector} missing menuitem children`,
            menu.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
    ...
    e.preventDefault();
        ...
        issues.push(
          this.createIssue(
            'incomplete-menu-pattern',
            'warning',
            `Menu at ${elementSelector} missing arrow key navigation`,
            menu.location,
            ['2.1.1'],
            context,
            { fix }
      ...
      <div role="treeitem">Child Item</div>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Tree at ${elementSelector} missing treeitem children`,
            tree.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
      ...
      current.setAttribute('aria-expanded', 'false');
        ...
        issues.push(
          this.createIssue(
            'incomplete-tree-pattern',
            'warning',
            `Tree at ${elementSelector} missing arrow key navigation`,
            tree.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  private validateToolbarPattern(
    ...
    for (const toolbar of toolbars) {
      ...
      const hasRovingTabindex = nodes.some(
        (n) =>
          n.actionType === 'ariaStateChange' &&
...
for (let i = 1; i < buttons.length; i++) {
  buttons[i].setAttribute('tabindex', '-1');
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Toolbar at ${elementSelector} missing roving tabindex pattern`,
            toolbar.location,
            ['2.1.1'],
            context,
            { fix }
      ...
      buttons[nextIndex].focus();
        ...
        issues.push(
          this.createIssue(
            'incomplete-toolbar-pattern',
            'warning',
            `Toolbar at ${elementSelector} missing arrow key navigation`,
            toolbar.location,
            ['2.1.1'],
            context,
            { fix }
    ...
    <div role="gridcell">Cell 2,2</div>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Grid at ${elementSelector} missing row children`,
            grid.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
      ...
      const has2DNavigation = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
          n.event === 'keydown' &&
    ...
    if (prevRow) prevRow.querySelectorAll('[role="gridcell"]')[cellIndex]?.focus();
        ...
        issues.push(
          this.createIssue(
            'incomplete-grid-pattern',
            'warning',
            `Grid at ${elementSelector} missing 2D arrow key navigation`,
            grid.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  <div role="option" aria-selected="true">Option 3</div>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Listbox at ${elementSelector} missing option children`,
            listbox.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
    ...
    options[(currentIndex - 1 + options.length) % options.length]?.focus();
        ...
        issues.push(
          this.createIssue(
            'incomplete-listbox-pattern',
            'warning',
            `Listbox at ${elementSelector} missing arrow key navigation`,
            listbox.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  <div role="radio" aria-checked="false">Option 3</div>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Radiogroup at ${elementSelector} missing radio children`,
            radiogroup.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
    ...
    radios[nextIndex].focus();
        ...
        issues.push(
          this.createIssue(
            'incomplete-radiogroup-pattern',
            'warning',
            `Radiogroup at ${elementSelector} missing arrow key navigation`,
            radiogroup.location,
            ['2.1.1'],
            context,
            { fix }
        ...
        if (!hasAttr) {
          ...
          issues.push(
            this.createIssue(
              ...
              'error',
              `Slider at ${elementSelector} missing required ${attr}`,
              slider.location,
              ['4.1.2'],
              context,
              { fix }
  ...
  slider.setAttribute('aria-valuetext', value + '%');
        ...
        issues.push(
          this.createIssue(
            'incomplete-slider-pattern',
            'warning',
            `Slider at ${elementSelector} missing arrow key handlers`,
            slider.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  private validateSpinbuttonPattern(
    ...
    for (const spinbutton of spinbuttons) {
        ...
        if (!hasAttr) {
          ...
          issues.push(
            this.createIssue(
              ...
              'error',
              `Spinbutton at ${elementSelector} missing required ${attr}`,
              spinbutton.location,
              ['4.1.2'],
              context,
              { fix }
      ...
      const hasUpDownKeys = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
          n.event === 'keydown' &&
          n.element.selector === elementSelector &&
  ...
  spinbutton.setAttribute('aria-valuenow', String(value));
        ...
        issues.push(
          this.createIssue(
            'incomplete-spinbutton-pattern',
            'warning',
            `Spinbutton at ${elementSelector} missing Up/Down arrow key handlers`,
            spinbutton.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  private validateSwitchPattern(
    ...
    for (const switchElem of switches) {
      ...
      const hasAriaChecked = nodes.some(
        (n) =>
          n.actionType === 'ariaStateChange' &&
      ...
      if (!hasAriaChecked) {
        ...
        issues.push(
          this.createIssue(
            ...
            'error',
            `Switch at ${elementSelector} missing aria-checked attribute`,
            switchElem.location,
            ['4.1.2'],
            context,
            { fix }
...
switchElem.addEventListener('keydown', (e) => {
    ...
    e.preventDefault();
        ...
        issues.push(
          this.createIssue(
            'incomplete-switch-pattern',
            'warning',
            `Switch at ${elementSelector} missing toggle handler`,
            switchElem.location,
            ['2.1.1'],
            context,
            { fix }
  ...
  private validateBreadcrumbPattern(
    ...
    const navElements = nodes.filter(
      (n) =>
        n.actionType === 'ariaStateChange' &&
        n.metadata.attribute === 'role' &&
    ...
    for (const nav of navElements) {
      ...
      const hasBreadcrumbLabel = nodes.some(
        (n) =>
          n.actionType === 'ariaStateChange' &&
      ...
      if (hasBreadcrumbLabel) {
        ...
        const hasAriaCurrent = nodes.some(
          (n) =>
            n.actionType === 'ariaStateChange' &&
    ...
    <li><span aria-current="page">Product Details</span></li>
          ...
          issues.push(
            this.createIssue(
              ...
              'info',
              `Breadcrumb navigation missing aria-current="page" on last item`,
              nav.location,
              ['1.3.1'],
              context,
              { fix }
    ...
    <p>Content...</p>
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Feed at ${elementSelector} missing article children`,
            feed.location,
            ['4.1.2', '1.3.1'],
            context,
            { fix }
  ...
  article.setAttribute('aria-setsize', String(totalArticles));
        ...
        issues.push(
          this.createIssue(
            'incomplete-feed-pattern',
            'info',
            `Feed at ${elementSelector} missing aria-posinset/aria-setsize on articles`,
            feed.location,
            ['4.1.2'],
            context,
            { fix }
  ...
  private validateDisclosurePattern(
    ...
    for (const disclosure of disclosures) {
      ...
      const hasButtonRole = nodes.some(
        (n) =>
          n.actionType === 'ariaStateChange' &&
          n.metadata.attribute === 'role' &&
...
<button aria-expanded="false">Toggle</button>`,
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Element at ${elementSelector} with aria-expanded should be a button or have role="button"`,
            disclosure.location,
            ['4.1.2'],
            context,
            { fix }
  ...
  private validateCarouselPattern(
    ...
    const hasAutoRotation = nodes.some(
      (n) =>
        n.actionType === 'domManipulation' &&
    ...
    if (hasAutoRotation) {
      ...
      const hasPauseControl = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
      ...
      if (!hasPauseControl) {
        const firstAutoRotation = nodes.find(
          (n) =>
            n.actionType === 'domManipulation' &&
            (n.metadata.method === 'setInterval' ||
              n.metadata.method === 'setTimeout')
...
pauseButton.addEventListener('click', () => {
    ...
    pauseButton.setAttribute('aria-label', 'Pause carousel');
          ...
          issues.push(
            this.createIssue(
              ...
              'warning',
              'Auto-rotating carousel missing pause/play control',
              firstAutoRotation.location,
              ['2.2.2'],
              context,
              { fix }
            )
  ...
  private validateLinkPattern(
    ...
    for (const link of links) {
      ...
      const hasClickHandler = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
          n.event === 'click' &&
          n.element.selector === elementSelector
...
<a href="/destination">Link text</a>`,
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Element with role="link" at ${elementSelector} missing click handler`,
            link.location,
            ['2.1.1'],
            context,
            { fix }
      ...
      const hasEnterHandler = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
          n.event === 'keydown' &&
          n.handler?.body?.includes('Enter') &&
    ...
    window.location.href = element.getAttribute('href');
        ...
        issues.push(
          this.createIssue(
            'incomplete-link-pattern',
            'info',
            `Element with role="link" at ${elementSelector} missing Enter key handler`,
            link.location,
            ['2.1.1'],
            context,
            { fix }
        ...
        if (!hasAttr) {
          ...
          issues.push(
            this.createIssue(
              ...
              'error',
              `Meter at ${elementSelector} missing required ${attr}`,
              meter.location,
              ['4.1.2'],
              context,
              { fix }
...
// No aria-valuenow needed, just role="progressbar"`,
        ...
        issues.push(
          this.createIssue(
            ...
            'info',
            `Progressbar at ${elementSelector} missing aria-valuenow (add if progress is determinate)`,
...
triggerElement.setAttribute('aria-describedby', 'tooltip-1');`,
        ...
        issues.push(
          this.createIssue(
            ...
            'warning',
            `Tooltip at ${elementSelector} missing unique id for aria-describedby reference`,
            tooltip.location,
            ['4.1.2'],
            context,
            { fix }
      ...
      const hasShowHideHandlers = nodes.some(
        (n) =>
          n.actionType === 'eventHandler' &&
          (n.event === 'mouseenter' ||
            n.event === 'mouseleave' ||
  ...
  tooltip.hidden = true;
        ...
        issues.push(
          this.createIssue(
            'incomplete-tooltip-pattern',
            'info',
            `Tooltip at ${elementSelector} missing show/hide handlers on trigger`,
            tooltip.location,
            ['4.1.2'],
            context,
            { fix }
